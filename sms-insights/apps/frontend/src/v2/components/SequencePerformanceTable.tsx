...
function SequenceFamilyGroup({
  ...
  return (
    <div
      className="V2Panel V2Panel--nested"
      style={{
              ...
              >
                ...
                {columnVisibility.gaps && (
                  <td style={{ textAlign: 'right', paddingRight: '1.25rem' }}>
                    {gaps.length > 0 ? (
                      <div
                        className="V2Table__cell--gap-warning"
                        title={`${gaps.length} booked calls almost matched this version but were below the fuzzy threshold.`}
                      >
                        ⚠️ {gaps.length} Gaps
                      </div>
                    ) : (
