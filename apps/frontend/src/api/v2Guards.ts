...
export function assertWeeklySummaryV2Envelope(value: unknown): asserts value is ApiEnvelope<WeeklyManagerSummaryV2> {
  ...
  if (!isString(data.sources.monday.status))
    throw new Error('Invalid v2 weekly-summary response: sources.monday.status');
  if (!isBoolean(data.sources.monday.enabled))
    throw new Error('Invalid v2 weekly-summary response: sources.monday.enabled');
  ...
  if (!isNumber(data.teamTotals.messagesSent))
    throw new Error('Invalid v2 weekly-summary response: teamTotals.messagesSent');
  ...
  if (!Array.isArray(data.actionsNextWeek))
    throw new Error('Invalid v2 weekly-summary response: actionsNextWeek missing');
}
...
export function assertMondayLeadInsightsV2Envelope(value: unknown): asserts value is ApiEnvelope<MondayLeadInsightsV2> {
  ...
  if (!Array.isArray(data.activityByDay))
    throw new Error('Invalid v2 monday lead-insights response: activityByDay missing');
...
export function assertMondayScorecardsV2Envelope(value: unknown): asserts value is ApiEnvelope<MondayScorecardsV2> {
  ...
  if (!Array.isArray(value.data.trendByDay))
    throw new Error('Invalid v2 monday scorecards response: trendByDay missing');
...
export function assertDraftSuggestionEnvelope(value: unknown): asserts value is ApiEnvelope<DraftSuggestionV2> {
  ...
  if (!isString(value.data.generationMode))
    throw new Error('Invalid draft suggestion response: generationMode missing');
...
export function assertCrmNotesSuggestionEnvelope(value: unknown): asserts value is ApiEnvelope<{
...
export function assertSequenceKpisV2Envelope(value: unknown): asserts value is ApiEnvelope<SequenceKpisV2> {
  ...
  if (!isNumber(verification.slackBookedTotal))
    throw new Error('Invalid sequence KPIs response: verification.slackBookedTotal must be number');
  if (!isNumber(verification.matchedCalls))
    throw new Error('Invalid sequence KPIs response: verification.matchedCalls must be number');
  if (!isNumber(verification.unattributedCalls))
    throw new Error('Invalid sequence KPIs response: verification.unattributedCalls must be number');
  if (!isNumber(verification.manualDirectBooked))
    throw new Error('Invalid sequence KPIs response: verification.manualDirectBooked must be number');
  if (!isNumber(verification.manualDirectSharePct))
    throw new Error('Invalid sequence KPIs response: verification.manualDirectSharePct must be number');
  if (!isNumber(verification.smsPhoneMatchedCalls))
    throw new Error('Invalid sequence KPIs response: verification.smsPhoneMatchedCalls must be number');
  if (!isNumber(verification.fuzzyTextMatchedCalls))
    throw new Error('Invalid sequence KPIs response: verification.fuzzyTextMatchedCalls must be number');
}
...
export function assertAttributionHealthV2Envelope(value: unknown): asserts value is ApiEnvelope<AttributionHealthV2> {
  ...
  if (!isNullableString(data.maxBookedCallsTs))
    throw new Error('Invalid attribution health response: maxBookedCallsTs must be string or null');
  if (!isNullableString(data.maxAttributionTs))
    throw new Error('Invalid attribution health response: maxAttributionTs must be string or null');
  ...
  if (!isNumber(data.openReviewItems))
    throw new Error('Invalid attribution health response: openReviewItems must be number');
  if (!isNumber(data.unresolvedAttributions))
    throw new Error('Invalid attribution health response: unresolvedAttributions must be number');
}
...
export function assertRepResponseV2Envelope(value: unknown): asserts value is ApiEnvelope<RepResponseDailyRowV2[]> {
...
export function assertSequenceVersionHistoryV2Envelope(
  ...
  if (!Array.isArray(value.data.items))
    throw new Error('Invalid sequence version history response: items must be array');
...
export function assertChangelogEnvelope(value: unknown): asserts value is ApiEnvelope<ChangelogTimeline> {
  ...
  if (!isString(value.data.dateRange.from))
    throw new Error('Invalid changelog response: dateRange.from must be string');
  ...
  if (!isNumber(value.data.stats.features))
    throw new Error('Invalid changelog response: stats.features must be number');
  ...
  if (!isNumber(value.data.stats.refactors))
    throw new Error('Invalid changelog response: stats.refactors must be number');
