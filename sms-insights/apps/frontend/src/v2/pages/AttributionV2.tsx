...
export default function AttributionV2() {
  ...
  if (isLoading)
    return <V2State kind="loading">Loading booking source details…</V2State>;
  if (isError || !payload) {
    return (
      <V2State kind="error">
        Could not load booking source details:{' '}
    ...
    <div className="V2Page V2PageTransition V2Page--clean">
      ...
      <section className="V2MetricsGrid">
        <V2MetricCard
          ...
          value={payload.bookedCredit.total.toLocaleString()}
          tone="positive"
        />
        <V2MetricCard
          label="Overall reply rate"
          value={`${payload.totals.replyRatePct.toFixed(1)}%`}
        />
        <V2MetricCard
          label={<V2Term term="manualReplyRate" />}
          value={`${payload.totals.manualReplyRatePct.toFixed(1)}%`}
        />
        <V2MetricCard
          label={<V2Term term="sequenceReplyRate" />}
          value={`${payload.totals.sequenceReplyRatePct.toFixed(1)}%`}
        />
      </section>

      <div className="V2Grid V2Grid--2">
        <V2Panel
          title="Booked Calls Source (Slack)"
          caption="This is the main booked-call number and credit source."
        >
          <ul className="V2BulletList">
            <li>
              Source: Slack booked-call channel plus reaction-based credit
              routing.
            </li>
            <li>
              This is the booked-call number used in scorecards and weekly
              reviews.
            </li>
        ...
        <V2Panel
          title="How Booking Credit Works"
          caption="Clear rule set for how setter credit is assigned."
        >
          <ul className="V2BulletList">
      ...
      <V2Panel
        title="Sequence Match Summary"
        caption='How many booked calls matched a sequence name vs "No sequence (manual/direct)".'
      >
        <div className="V2SplitStat">
          <div>
            <span>Total Calls Booked</span>
            <strong>
              {payload.provenance.sequenceBookedAttribution?.totalCalls ?? 0}
            </strong>
          </div>
          <div>
            <span>Matched to sequence</span>
            <strong>
              {payload.provenance.sequenceBookedAttribution?.matchedCalls ?? 0}
            </strong>
          </div>
          <div>
            <span>Manual/Direct</span>
            <strong>
              {payload.provenance.sequenceBookedAttribution?.manualCalls ?? 0}
            </strong>
          </div>
          <div>
