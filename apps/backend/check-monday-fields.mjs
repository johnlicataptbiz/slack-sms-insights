...
async function checkMissingFields() {
  ...
  columns.forEach((col, idx) => {
    const isMapped =
      Object.values(dbMapping || {}).includes(col.id) || Object.values(envMapping || {}).includes(col.id);
...
async function main() {
  try {
    ...
    console.log(
      `${colors.bright}${colors.cyan}║       MONDAY BOARD FIELD MAPPING DIAGNOSTIC                   ║${colors.reset}`,
    );
    console.log(
      `${colors.bright}${colors.cyan}╚═══════════════════════════════════════════════════════════════╝${colors.reset}`,
    );
