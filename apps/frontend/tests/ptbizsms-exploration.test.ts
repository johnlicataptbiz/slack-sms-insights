...
test.describe('PTBiz SMS Insights - UI Modernization Exploration', () => {
  ...
  test('2. Dark Theme Verification - Color Palette', async ({ page }) => {
    test.info().annotations.push({
      type: 'feature',
      description: 'Dark theme colors (#0f1419 navy, #00d9ff cyan) are rendered correctly'
    });
    ...
    const hasDesignTokens = Object.values(cssVars).some(v => v && v.trim().length > 0);
  ...
  test('3. Navigation - Dashboard Structure', async ({ page }) => {
    ...
    const navElements = page.locator('nav, [role="navigation"], header, [class*="nav"]');
    ...
    const mainContent = page.locator('main, [role="main"], [class*="content"], [class*="dashboard"]');
  ...
  test('4. Metrics Display - Card Components', async ({ page }) => {
    ...
    const cards = page.locator('[class*="card"], [class*="metric"], [role="article"], div[class*="Box"]');
  ...
  test('6. Responsive Design - Mobile Viewport', async ({ page }) => {
    ...
    const responsiveElements = page.locator('[class*="mobile"], [class*="responsive"], [class*="menu"]');
  ...
  test('8. Accessibility - Keyboard Navigation', async ({ page }) => {
    ...
    const isFocused = await passwordInput.first().evaluate(el => el === document.activeElement);
  ...
  test('9. Typography - New Font System', async ({ page }) => {
    test.info().annotations.push({
      type: 'design',
      description: 'Dashboard uses new typography system (Plus Jakarta Sans, Lexend Deca)'
    });
  ...
  test('10. CSS Modernization - Component Classes', async ({ page }) => {
    test.info().annotations.push({
      type: 'design',
      description: 'Dashboard uses V2 component architecture with modernized styles'
    });
    ...
    const shadowElements = await page.locator('[class*="shadow"], [class*="dark"]').count();
...
test.describe('Production Deployment Verification', () => {
  ...
  test('CSS Bundle - Modernization Assets Loaded', async ({ page }) => {
    test.info().annotations.push({
      type: 'deployment',
      description: 'All CSS assets from modernization (v2.css, enhancements.css) are loaded'
    });
