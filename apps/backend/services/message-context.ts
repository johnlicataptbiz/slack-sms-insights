...
const MESSAGE_LIMIT = Number.parseInt(process.env.MESSAGE_CONTEXT_LIMIT || '100', 10);
const CACHE_TTL_MS = Number.parseInt(process.env.CONTEXT_CACHE_TTL_MINUTES || '60', 10) * 60 * 1000;

// Global cache
const globalContext: GlobalTeamContext = {
  channels: new Map(),
  allUsers: new Map(),
  lastRefresh: 0,
};
 ...
 */
export const loadChannelContext = async (app: App, channelId: string, channelName: string): Promise<ChannelContext> => {
  try {
    ...
    for (const [userId, data] of userMessages) {
      // Get user info
      const userInfo = await app.client.users.info({ user: userId }).catch(() => null);

      userContexts.set(userId, {
        userId,
        displayName: userInfo?.user?.profile?.display_name || userInfo?.user?.name || userId,
    ...
    const messagePatterns = {
      ...
      updates: messages.filter((m) => m.text?.match(/\b(update|updated|synced|complete)\b/i)).length,
      jokes: messages.filter((m) => m.text?.match(/😂|😄|lol|haha/i)).length,
      complaints: messages.filter((m) => m.text?.match(/\b(broken|error|issue|problem|help)\b/i)).length,
    };
    ...
    logger.app.info(`[message-context] Loaded context for ${userContexts.size} users in #${channelName}`);
 ...
 */
function deriveActivityLevel(count: number, limit: number): 'quiet' | 'normal' | 'active' | 'chatty' {
 ...
 */
export const getUserContextInChannel = (channelId: string, userId: string): UserContext | undefined => {
 ...
 * Get channel context
 */
export const getChannelContext = (channelId: string): ChannelContext | undefined => {
 ...
 */
export const refreshTeamContext = async (app: App, channelIds: string[]): Promise<Map<string, ChannelContext>> => {
  ...
  try {
    ...
    for (const channelId of channelIds) {
      // Get channel info
      const channelInfo = await app.client.conversations.info({ channel: channelId }).catch(() => null);
 ...
 */
export const formatChannelContextForDerek = (channelId: string, includeUsers = 3): string => {
  ...
  const userSummary = topUsers.map((u) => `${u.displayName} (${u.activityLevel})`).join(', ');
