...
async function getBookedCallStats() {
  ...
  recentCalls.forEach((call, idx) => {
    ...
    if (call.first_sms_touch_at) {
      const touchDiff = Math.floor((new Date(call.event_ts) - new Date(call.first_sms_touch_at)) / (1000 * 60 * 60 * 24));
...
async function getSequencePerformance() {
  ...
  sequences.forEach((seq, idx) => {
    ...
    if (total > 0) {
      ...
      log.data(`Total: ${fmt(total)} | Outbound: ${fmt(outbound)} | Inbound: ${fmt(inbound)} | Reply Rate: ${total > 0 ? ((inbound/outbound * 100).toFixed(1)) : 0}%`);
    }
  });
}
...
async function getContactQuality() {
  ...
  recentContacts.forEach((contact, idx) => {
    ...
    log.data(`Source: ${contact.lead_source || 'N/A'} | Interest: ${contact.coaching_interest} | Employment: ${contact.employment_status}`);
  });
}
...
async function getDailyMetrics() {
  ...
  Object.entries(byDate).forEach(([date, data]) => {
    ...
    log.data(`${date.padEnd(15)} Total: ${fmt(total).padEnd(8)} | Out: ${fmt(data.outbound).padEnd(8)} | In: ${fmt(data.inbound).padEnd(8)} | Reply Rate: ${data.outbound > 0 ? ((data.inbound/data.outbound * 100).toFixed(1)) : 0}%`);
  });
}
...
async function main() {
  try {
    ...
    console.log(`${colors.bright}${colors.magenta}║                    SMS INSIGHTS DATABASE EXPLORER                             ║${colors.reset}`);
