  ...
  for (const [name, items] of duplicates) {
    ...
    for (const manualItem of manual) {
      for (const trackedItem of tracked) {
        ...
        const trackedCols = new Map(trackedItem.column_values?.map((c) => [c.id, c]) || []);
        const manualCols = new Map(manualItem.column_values?.map((c) => [c.id, c]) || []);
        ...
        if (differences.length === 0) {
          ...
          const onlyEmptyDiffs = differences.every((d) => d.tracked === '(empty)' || d.manual === '(empty)');
