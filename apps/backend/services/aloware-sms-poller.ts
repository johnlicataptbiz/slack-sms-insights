...
const getPollIntervalMs = (): number => {
  return Number.parseInt(process.env.ALOWARE_POLL_INTERVAL_MS || String(DEFAULT_POLL_INTERVAL_MS), 10);
};

const getLookbackMinutes = (): number => {
  return Number.parseInt(process.env.ALOWARE_POLL_LOOKBACK_MINUTES || String(DEFAULT_LOOKBACK_MINUTES), 10);
};

const getMaxEventsPerPoll = (): number => {
  return Number.parseInt(process.env.ALOWARE_POLL_MAX_EVENTS || String(DEFAULT_MAX_EVENTS_PER_POLL), 10);
};
...
const normalizeDirection = (direction?: string): 'inbound' | 'outbound' | 'unknown' => {
...
const fetchAlowareSmsEvents = async (
  ...
  logger?.debug?.('Fetching SMS events from Aloware API', { url: url.replace(token, '***') });
  ...
  const data: AlowareApiResponse = await response.json();
...
export const pollAlowareSmsEvents = async (
  ...
  if (state.consecutiveErrors >= 5) {
    logger?.warn('Aloware polling paused due to consecutive errors', { consecutiveErrors: state.consecutiveErrors });
    return { ingested: 0, errors: 0, total: 0 };
  }
  ...
  try {
    const events = await fetchAlowareSmsEvents({ since, limit: getMaxEventsPerPoll() }, logger);
...
export const getAlowarePollingState = (): PollState & { enabled: boolean; intervalMs: number } => {
  return {
    ...state,
