...
async function checkForDuplicates() {
  ...
  recentSyncs.forEach((sync, idx) => {
    ...
    log.data(
      `${idx + 1}. ${marker} Item ${sync.monday_item_id} - Pushed: ${new Date(sync.pushed_at).toLocaleString()}`,
    );
  });
...
async function checkPendingDetails() {
  ...
  const pushMap = new Map(pushes.map((p) => [`${p.slack_channel_id}:${p.slack_message_ts}`, p]));
...
async function main() {
  try {
    ...
    console.log(
      `${colors.bright}${colors.cyan}║           MONDAY BOARD DUPLICATE CHECK & ANALYSIS             ║${colors.reset}`,
    );
    console.log(
      `${colors.bright}${colors.cyan}╚═══════════════════════════════════════════════════════════════╝${colors.reset}`,
    );
