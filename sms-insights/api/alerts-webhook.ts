    ...
    try {
      ...
      switch (payload.checkType) {
        ...
        default:
          res
            .status(400)
            .json({ error: `Unknown checkType: ${payload.checkType}` });
          return;
      }
...
alertRouter.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { critical, stale, unassigned, needsReply } = req.body as Record<
        string,
        number
      >;
...
alertRouter.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { lagHours, maxBookedCallsTs, maxAttributionTs } =
...
async function triggerProactiveAlertsWorkflow(
...
async function triggerInboxWatchWorkflow(
  payload: AlertWebhookPayload,
): Promise<void> {
...
async function triggerAttributionHealthWorkflow(
  payload: AlertWebhookPayload,
): Promise<void> {
