...
const BOT_PURPOSE =
  process.env.BOT_PURPOSE || "Your friendly SMS analytics dude";
const BOT_EMOJI = process.env.BOT_EMOJI || "📊";
const BOT_PERSONALITY =
  process.env.BOT_PERSONALITY ||
  "chill, direct, a little sarcastic, gets excited about metrics";

const MIN_POST_INTERVAL_MS =
  parseInt(process.env.DEREK_MIN_POST_INTERVAL_MIN || "30", 10) * 60 * 1000;
const DEREK_POST_CONFIDENCE_THRESHOLD = parseFloat(
  process.env.DEREK_CONFIDENCE_THRESHOLD || "0.65",
);
const DEREK_MAX_POSTS_PER_DAY = parseInt(
...
export const canDerekPost = (
  ...
  if (timeSinceLastPost < MIN_POST_INTERVAL_MS) {
    const minutesUntilReady = Math.ceil(
      (MIN_POST_INTERVAL_MS - timeSinceLastPost) / 60000,
    );
    return {
      canPost: false,
...
export const getDerekCommentIfWorth = (
...
export const derekResponds = async (
  ...
  if (cleanMsg.includes("slow")) {
    const choice =
      roastTemplates.slowResponse[
        Math.floor(Math.random() * roastTemplates.slowResponse.length)
      ];
    return `${BOT_EMOJI} ${choice}`;
  }
  if (cleanMsg.includes("fast") || cleanMsg.includes("quick")) {
    const choice =
      roastTemplates.fastResponse[
        Math.floor(Math.random() * roastTemplates.fastResponse.length)
      ];
    return `${BOT_EMOJI} ${choice}`;
  }
...
export const postDerekComment = async (
  ...
  try {
    ...
    if (!message) {
      logger.app.debug(
        `[derek] Message didn't meet confidence threshold for ${channelId}`,
      );
      return false;
    }
    ...
    derekPostTracker.dailyPostCount.set(
      channelId,
      (derekPostTracker.dailyPostCount.get(channelId) || 0) + 1,
    );
