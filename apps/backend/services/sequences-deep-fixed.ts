...
export const getSequencesDeep = async (
  ...
  try {
    ...
    logger?.warn?.(
      "sequences-deep-fixed: factMondayHealthDaily unavailable",
      e,
    );
  }
  ...
  const manualSequenceId =
    sequenceRows?.find((row: any) => row.isManualBucket)?.id || null;
  ...
  for (const event of events) {
    if (event.event_ts < params.from || event.direction !== "outbound")
      continue;
    ...
    sequenceRows
      ...
      .map((row: any) => {
        ...
        return {
          ...
          replyRatePct: stat
            ? (stat.repliesReceived / stat.uniqueContacted) * 100
            : 0,
          bookedCalls: 0,
          bookingRatePct: 0,
  ...
  const boards = mondayRows
    ? new Set(mondayRows.map((row: any) => row.boardId)).size
    : 0;
  const staleBoards = mondayRows
    ? mondayRows.filter((row: any) => row.isStale).length
    : 0;
  const erroredBoards = mondayRows
    ? mondayRows.filter((row: any) => row.syncStatus === "error").length
    : 0;
