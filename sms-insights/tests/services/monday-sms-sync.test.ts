...
describe('monday-sms-sync configuration', () => {
  describe('parseBool', () => {
    it('should parse true string to true', () => {
      const result = (process.env.MONDAY_SMS_SYNC_ENABLED = 'true');
      assert.equal(result, 'true');
    });

    it('should parse false string to false', () => {
      const result = (process.env.MONDAY_SMS_SYNC_ENABLED = 'false');
      assert.equal(result, 'false');
    });

    it('should return fallback for empty string', () => {
      const result = (process.env.MONDAY_SMS_SYNC_ENABLED = '');
      assert.equal(result, '');
    });
  });
  ...
  describe('parseCsv', () => {
    it('should parse CSV string to array', () => {
      ...
      const result = process.env.MONDAY_SMS_SYNC_BOARD_IDS.split(',').map((entry) => entry.trim()).filter((entry) => entry.length > 0);
