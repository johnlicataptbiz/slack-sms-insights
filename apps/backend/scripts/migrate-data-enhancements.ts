...
const parseArgs = () => {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--step' && args[i + 1]) {
      step = Number.parseInt(args[++i], 10);
...
const identifyRepFromLine = (line: string | null, alowareUser: string | null): string | null => {
  ...
  for (const phone of REP_CONFIG.jack.phones) {
    if (combined.includes(phone.replace(/\D/g, ''))) return 'jack';
  }
  for (const name of REP_CONFIG.jack.names) {
    if (combined.includes(name)) return 'jack';
  }
  for (const phone of REP_CONFIG.brandon.phones) {
    if (combined.includes(phone.replace(/\D/g, ''))) return 'brandon';
  }
  for (const name of REP_CONFIG.brandon.names) {
    if (combined.includes(name)) return 'brandon';
  }
  return null;
};
      ...
      try {
        ...
        if (repId) {
          if (dryRun) logger.info(`[DRY-RUN] Would assign rep "${repId}" to ${row.id} (line: ${row.line})`);
            ...
            await client.query('UPDATE conversations SET current_rep_id = $1, updated_at = NOW() WHERE id = $2', [
              repId,
              row.id,
            ]);
          updated++;
...
async function expandQualification(dryRun: boolean, limit?: number) {
  ...
  try {
    const result = await client.query(
      `SELECT DISTINCT c.id, c.contact_key, c.contact_id FROM conversations c LEFT JOIN conversation_state cs ON cs.conversation_id = c.id WHERE c.status = 'open' AND (cs.conversation_id IS NULL OR cs.qualification_full_or_part_time = 'unknown') AND EXISTS (SELECT 1 FROM sms_events se WHERE se.conversation_id = c.id AND se.direction = 'inbound' AND se.body IS NOT NULL) ${limit ? `LIMIT ${limit}` : ''}`,
    );
    for (const conv of result.rows) {
      try {
        ...
        const res = await syncQualificationFromConversationText(
          {
...
async function cleanNicheData(dryRun: boolean, limit?: number) {
  ...
  try {
    const result = await client.query(
      `SELECT contact_key, niche FROM inbox_contact_profiles WHERE niche IS NOT NULL AND niche != '' ${limit ? `LIMIT ${limit}` : ''}`,
    );
    for (const row of result.rows) {
      const normalized = categorizeNiche(row.niche);
      if (normalized && normalized !== row.niche) {
        if (!dryRun)
          await client.query('UPDATE inbox_contact_profiles SET niche = $1 WHERE contact_key = $2', [
            normalized,
            row.contact_key,
          ]);
        updated++;
...
async function activateEscalation(dryRun: boolean, limit?: number) {
  ...
  try {
    const result = await client.query(
      `SELECT conversation_id, escalation_level, cadence_status, last_podcast_sent_at, qualification_coaching_interest FROM conversation_state ${limit ? `LIMIT ${limit}` : ''}`,
    );
    for (const row of result.rows) {
      try {
        const messages = await listMessagesForConversation(row.conversation_id, 100, logger);
        const esc = classifyEscalationLevel(messages, {
          escalation_level: row.escalation_level,
          qualification_coaching_interest: row.qualification_coaching_interest,
        });
        ...
        if (esc.level !== row.escalation_level || newCadence !== row.cadence_status) {
          if (!dryRun)
            await client.query(
              'UPDATE conversation_state SET escalation_level = $2, escalation_reason = $3, cadence_status = $4 WHERE conversation_id = $1',
              [row.conversation_id, esc.level, esc.reason, newCadence],
            );
          updated++;
        }
      } catch (e) {
    ...
    for (const row of result.rows) {
      try {
        const stateRes = await client.query(
          'SELECT escalation_level FROM conversation_state WHERE conversation_id = $1',
          [row.conversation_id],
        );
        ...
        const sig =
          [
            /\?/.test(body) ? 'Q' : '',
            /\b(call|book|schedule)\b/i.test(body) ? 'CTA' : '',
            /\b(you|your|we)\b/i.test(body) ? 'P' : '',
          ]
            .filter(Boolean)
            .join('-') || 'simple';
        if (!dryRun)
          await client.query(
            `INSERT INTO conversion_examples (source_outbound_event_id, booked_call_label, escalation_level, structure_signature, channel_marker) VALUES ($1,$2,$3,$4,'sms') ON CONFLICT DO NOTHING`,
            [row.id, row.sequence || 'manual', level, sig],
          );
        added++;
      } catch (e) {
        errors++;
...
async function addWorkItemTypes(dryRun: boolean, limit?: number) {
  ...
  try {
    const followUps = await client.query(
      `SELECT c.id, c.current_rep_id FROM conversations c JOIN conversation_state cs ON cs.conversation_id = c.id WHERE cs.qualification_coaching_interest = 'high' AND c.status = 'open' AND c.last_touch_at < NOW() - INTERVAL '3 days' AND NOT EXISTS (SELECT 1 FROM work_items wi WHERE wi.conversation_id = c.id AND wi.type = 'follow_up' AND wi.resolved_at IS NULL) ${limit ? `LIMIT ${limit}` : ''}`,
    );
    for (const row of followUps.rows) {
      if (!dryRun)
        await client.query(
          `INSERT INTO work_items (type, conversation_id, rep_id, severity, due_at) VALUES ('follow_up',$1,$2,'med',NOW() + INTERVAL '2 days')`,
          [row.id, row.current_rep_id],
        );
      added++;
    }
    const hotLeads = await client.query(
      `SELECT c.id, c.current_rep_id FROM conversations c JOIN conversation_state cs ON cs.conversation_id = c.id WHERE cs.escalation_level = 3 AND c.status = 'open' AND NOT EXISTS (SELECT 1 FROM work_items wi WHERE wi.conversation_id = c.id AND wi.type = 'hot_lead' AND wi.resolved_at IS NULL) ${limit ? `LIMIT ${limit}` : ''}`,
    );
    for (const row of hotLeads.rows) {
      if (!dryRun)
        await client.query(
          `INSERT INTO work_items (type, conversation_id, rep_id, severity, due_at) VALUES ('hot_lead',$1,$2,'high',NOW() + INTERVAL '4 hours')`,
          [row.id, row.current_rep_id],
        );
      added++;
    }
