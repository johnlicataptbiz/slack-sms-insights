...
async function addMissingCalls() {
  ...
  const mapSourceToMondaySource = (firstConversion) => {
    ...
    if (normalized.includes('checklist') || normalized.includes('startup')) return 'Start-Up Checklist';
    if (normalized.includes('rates') || normalized.includes('10k months')) return 'Raise Your Rates';
    ...
    if (normalized.includes('webinar') || normalized.includes('challenge')) return 'Webinar';
...
addMissingCalls().catch(console.error);
