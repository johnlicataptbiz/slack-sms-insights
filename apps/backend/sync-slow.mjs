...
async function syncOneAtATime() {
  ...
  const syncedMap = new Set(pushStatuses.map((p) => `${p.slack_channel_id}:${p.slack_message_ts}`));

  const pending = jackCalls.filter((c) => !syncedMap.has(`${c.slackChannelId}:${c.slackMessageTs}`));
