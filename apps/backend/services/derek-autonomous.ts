...
const DEREK_ENABLED = (process.env.DEREK_AUTONOMOUS_ENABLED ?? 'true').toLowerCase() === 'true';
const DEREK_POST_INTERVAL_MINUTES = Number.parseInt(process.env.DEREK_AUTONOMOUS_INTERVAL_MIN || '45', 10);
const DEREK_CONTEXT_REFRESH_INTERVAL_HOURS = Number.parseInt(process.env.DEREK_CONTEXT_REFRESH_HOURS || '1', 10);

// Target channels where Derek posts autonomously
const DEREK_TARGET_CHANNELS = (process.env.DEREK_TARGET_CHANNELS || 'C09ULGH1BEC').split(',');
  ...
  derekAutonomousIntervalId = setInterval(
    async () => {
      try {
        ...
        if (timeSinceRefresh > DEREK_CONTEXT_REFRESH_INTERVAL_HOURS * 60 * 60 * 1000) {
          try {
            await refreshTeamContext(app, DEREK_TARGET_CHANNELS);
            lastDerekContextRefresh = Date.now();
        ...
        for (const channelId of DEREK_TARGET_CHANNELS) {
          ...
          try {
            ...
            if (posted) {
              logger.app.debug(`[derek] Autonomously posted to ${channelIdTrimmed}`);
            } else {
              logger.app.debug(`[derek] Skipped post to ${channelIdTrimmed} (safeguards or low confidence)`);
            }
          } catch (error) {
