...
import { DEFAULT_BUSINESS_TIMEZONE } from "./services/time-range.js";
...
const shouldRefreshKpiFactsOnStartup = (): boolean => {
  const raw = (process.env.KPI_FACT_REFRESH_ON_STARTUP || "true")
    .trim()
    .toLowerCase();
  return raw !== "false" && raw !== "0" && raw !== "off";
};

const getKpiFactStartupLookbackDays = (): number => {
  const parsed = Number.parseInt(
    (process.env.KPI_FACT_REFRESH_LOOKBACK_DAYS || "45").trim(),
    10,
  );
  ...
  try {
    ...
    void (async () => {
      try {
        ...
        await ensureAnalyticsSchemaBaseline(app.logger);

        if (shouldRefreshKpiFactsOnStartup()) {
          const lookbackDays = getKpiFactStartupLookbackDays();
          const to = new Date();
          const from = new Date(
            to.getTime() - lookbackDays * 24 * 60 * 60 * 1000,
          );
          try {
            ...
            logger.app.warn(
              `[startup] KPI fact refresh failed: ${error instanceof Error ? error.message : String(error)}`,
            );
          }
        }
      } catch (error) {
    ...
    if (slackStarted) {
      const { startDailyReportCron, startLrnRefreshCron } =
