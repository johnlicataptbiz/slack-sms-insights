 ...
 */
export const getConversionTrends = async (days = 30, logger?: Logger): Promise<ConversionTrend[]> => {
  try {
    const prisma = getPrismaClient();
    const fromDate = new Date();
    fromDate.setDate(fromDate.getDate() - days);

    const conversationCounts = await prisma.$queryRawUnsafe<Array<{ date: string; count: bigint }>>(
      `
      SELECT 
    ...
    const bookedCallCounts = await prisma.$queryRawUnsafe<Array<{ date: string; count: bigint }>>(
      `
      SELECT 
    ...
    for (const date of Array.from(allDates).sort().reverse()) {
      ...
      const conversionRate = conversations > 0 ? (bookedCalls / conversations) * 100 : 0;
 ...
 */
export const getTeamPerformanceTrends = async (days = 30, logger?: Logger): Promise<TeamPerformanceTrend[]> => {
  try {
    const prisma = getPrismaClient();
    const fromDate = new Date();
    fromDate.setDate(fromDate.getDate() - days);
 ...
 */
export const getSLATrends = async (days = 30, logger?: Logger): Promise<SLAMetricsTrend[]> => {
  try {
    const prisma = getPrismaClient();
    const fromDate = new Date();
    fromDate.setDate(fromDate.getDate() - days);
    ...
    return data.map((row) => ({
      ...
      slaCompliance:
        Number(row.total_count) > 0 ? Math.round((Number(row.compliance_count) / Number(row.total_count)) * 100) : 0,
    }));
  } catch (error) {
    logger?.error(`[analytics-queries] getSLATrends failed: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
};
 ...
 */
export const getWeeklyWorkload = async (logger?: Logger): Promise<TrendDataPoint[]> => {
  try {
    const prisma = getPrismaClient();
    const fromDate = new Date();
    fromDate.setDate(fromDate.getDate() - 7);

    const data = await prisma.$queryRawUnsafe<Array<{ date: string; open_count: bigint; overdue_count: bigint }>>(
      `
      SELECT 
        DATE(c.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Chicago')::text as date,
 ...
 */
export const getLeadQualificationTrends = async (days = 30, logger?: Logger): Promise<TrendDataPoint[]> => {
  try {
    const prisma = getPrismaClient();
    const fromDate = new Date();
    fromDate.setDate(fromDate.getDate() - days);

    const data = await prisma.$queryRawUnsafe<Array<{ date: string; qual: bigint; unqual: bigint }>>(
      `
      SELECT 
        DATE(c.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Chicago')::text as date,
  ...
  try {
    ...
    return data.map((row) => {
      ...
      return {
        ...
        bookedCalls,
        totalConversations,
        conversionRate: totalConversations > 0 ? Math.round((bookedCalls / totalConversations) * 100 * 100) / 100 : 0,
        avgResponseTime: row.avg_response_time || 0,
      };
    });
  } catch (error) {
 ...
 */
export const getHourlyDistribution = async (days = 7, logger?: Logger): Promise<TrendDataPoint[]> => {
  try {
    const prisma = getPrismaClient();
    const fromDate = new Date();
    fromDate.setDate(fromDate.getDate() - days);

    const data = await prisma.$queryRawUnsafe<Array<{ hour: number; count: bigint }>>(
      `
      SELECT 
