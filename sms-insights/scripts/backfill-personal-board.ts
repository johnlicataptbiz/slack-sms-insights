...
const daysBack = daysArg ? parseInt(daysArg.split('=')[1], 10) : 7; // Default: last 7 days
...
async function main() {
  ...
  try {
    ...
    for (const call of bookedCalls) {
      ...
      const raw = call.raw as { contact_name?: string; contact_phone?: string; setter?: string; source?: string } | null;
