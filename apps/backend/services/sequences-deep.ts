...
export const getSequencesDeep = async (
  ...
  try {
    await prisma.fact_monday_health_daily.findMany({
      where: {
        day: {
  ...
  try {
    attributionStats = (await prisma.$queryRawUnsafe(
      `
      SELECT
        COUNT(*)::int AS total,
    ...
    )) as Array<{ total: number; mapped_conversation: number }>;
  } catch (e) {
  ...
  try {
    mondayBookedTotalRows = (await prisma.$queryRawUnsafe(
      `
      ...
      toDay,
      salesTeamBoardId,
    )) as Array<{ monday_booked_total: number }>;
  } catch (e) {
  ...
  try {
    await prisma.sms_events
      ...
      .then((rows: any[]) =>
        rows.map((row: any) => ({ ...row, event_ts: row.created_at }) as any),
      );
  } catch (e) {
  ...
  const manualSequenceId =
    sequenceRows.find((row: any) => row.is_manual_bucket)?.id || null;
  const backfillSequenceIds = new Set(
    sequenceRows
      .filter((row: any) => isMondayBackfillLabel(row.label))
      .map((row: any) => row.id),
  );
  ...
  const smsReplyLinks = await getBookedCallSmsReplyLinks(
    bookedCallSources,
  ).catch(() => new Map());
  const smsSequenceLookup = await getBookedCallSequenceFromSmsEvents(
    ...
    undefined,
    smsReplyLinks,
  ).catch(() => new Map() as Map<string, any>);
  ...
  return {
    ...
    monday: {
      ...
      avgSourceCoveragePct:
        mondayRows.length > 0
          ? mondayRows.reduce(
              (sum, row: any) => sum + row.source_coverage_pct,
              0,
            ) / mondayRows.length
          : 0,
      avgCampaignCoveragePct:
        mondayRows.length > 0
          ? mondayRows.reduce(
              (sum, row: any) => sum + row.campaign_coverage_pct,
              0,
            ) / mondayRows.length
          : 0,
      avgSetByCoveragePct:
        mondayRows.length > 0
          ? mondayRows.reduce(
              (sum, row: any) => sum + row.set_by_coverage_pct,
              0,
            ) / mondayRows.length
          : 0,
      avgTouchpointsCoveragePct:
        mondayRows.length > 0
          ? mondayRows.reduce(
              (sum, row: any) => sum + row.touchpoints_coverage_pct,
              0,
            ) / mondayRows.length
          : 0,
    },
