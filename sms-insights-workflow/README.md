...
### Safe Channel Targeting (Local vs Production)

SMS report triggers are now safe by default and will not create unless you pick
a channel explicitly.

For local/dev testing, set a non-production channel:

```zsh
$ SMS_REPORT_CHANNEL_ID=C0123456789 slack trigger create --trigger-def triggers/temp_verify_channel_reply_webhook_trigger.ts -a local
```

To intentionally target production, opt in explicitly:
