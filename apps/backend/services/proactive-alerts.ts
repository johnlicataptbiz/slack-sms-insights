...
export const calculateProgress = (
  ...
  const percentOfTarget = goal.max ? Math.max(0, 100 - (current / goal.target) * 100) : (current / goal.target) * 100;
...
export const buildAlertBlocks = (
  ...
  for (const alert of alerts) {
    const emoji =
      alert.type === 'critical' ? '🔴' : alert.type === 'warning' ? '🟡' : alert.type === 'opportunity' ? '💡' : 'ℹ️';
...
export const getCurrentMetricSnapshot = async (
  ...
  const [todayFacts] = await prisma.$queryRawUnsafe<
    Array<{
  ...
  const [historicalFacts] = await prisma.$queryRawUnsafe<
    Array<{
