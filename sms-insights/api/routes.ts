...
type RequestHandler = (req: IncomingMessage & { body?: any; user?: any }, res: ServerResponse, logger?: Pick<Logger, 'info' | 'debug' | 'warn' | 'error'>) => Promise<void>;
...
// OAuth endpoints
const handleOAuthStart: RequestHandler = async (req, res) => {
  const clientId = process.env.SLACK_CLIENT_ID;
  const redirectUri = process.env.DASHBOARD_AUTH_REDIRECT_URI || `http://localhost:${process.env.PORT || 3000}`;
  const scopes = ['users:read', 'chat:read'];
  const state = Math.random().toString(36).substring(7);

  if (!clientId) {
    return sendJson(res, 500, { error: 'SLACK_CLIENT_ID not configured' });
  }

  const url = new URL('https://slack.com/oauth/v2/authorize');
  url.searchParams.append('client_id', clientId);
  url.searchParams.append('scope', scopes.join(','));
  url.searchParams.append('redirect_uri', `${redirectUri}/api/oauth/callback`);
...
const handleOAuthCallback: RequestHandler = async (req, res, logger) => {
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return sendJson(res, 500, { error: 'OAuth credentials not configured' });
    }
...
const handleAuthVerify: RequestHandler = async (req, res) => {
  sendJson(res, 200, { ok: true, user: (req as any).user });
};

const handleGetRuns: RequestHandler = async (req, res, logger) => {
  const url = new URL(req.url || '', `http://${req.headers.host}`);
...
const handleGetRunById: RequestHandler = async (req, res, logger) => {
...
const handlePostRun: RequestHandler = async (req, res, logger) => {
  const botToken = req.headers['x-bot-token'];
  if (botToken !== process.env.SLACK_BOT_TOKEN) {
    return sendJson(res, 401, { error: 'Invalid bot token' });
  }
  ...
  try {
    ...
    if (!runId) {
      return sendJson(res, 500, { error: 'Failed to log run' });
    }
...
const handleGetChannels: RequestHandler = async (req, res, logger) => {
